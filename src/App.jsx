...
function App() {
  ...
  const loadDashboard = useCallback(async ({ refresh = false } = {}) => {
    ...
    try {
      const response = await fetch("/api/recalls/RB-2107", { cache: "no-store" });
      if (!response.ok) throw new Error("Unable to load recall data.");
      const payload = await response.json();
      setData(payload);
      setPlanStatus(payload.plan ? `Recall plan is ${payload.plan.status}.` : "");
      setStatus("ready");
    } catch {
      setStatus("error");
    } finally {
  ...
  useEffect(() => {
    const timer = window.setInterval(() => loadDashboard({ refresh: true }), 60000);
  ...
  const { recall, summary, regions, actions, affectedUnits, mode, trace, plan } = data;
  const visibleUnits = affectedUnits.filter((unit) => {
    if (queueFilter === "urgent") return unit.risk === "Urgent";
    if (queueFilter === "warehouse") return ["warehouse", "in_stock", "stored"].includes(String(unit.state).toLowerCase());
    return unit.recordType !== "warehouse";
  });
  const exportBrief = () => {
  ...
  const createPlan = async () => {
    ...
    try {
      ...
      setPlanStatus(`Recall plan saved as ${payload.plan.status}.`);
    } catch (error) { setPlanStatus(error.message); }
  };

  const maxRegion = Math.max(...regions.map((region) => region.customers), 1);
  return <div className="shell">
    <aside className="sidebar"><div className="brand"><span>R</span><div>RecallScope<small>RECALL INTELLIGENCE</small></div></div><nav><button className="active" onClick={() => navigateTo("workspace")}>Recall workspace</button><button onClick={() => navigateTo("impact")}>Recall history</button><button onClick={() => navigateTo("trace-result")}>Supply network</button><button onClick={() => navigateTo("action-plan")}>Action plans</button></nav><div className="sidebar-note"><span className="signal"/> LIVE WATCH<br/><strong>1 active investigation</strong></div><div className="profile"><div className="avatar">AM</div><div><strong>Anil Menon</strong><small>Operations lead</small></div></div></aside>
    <main className="content" id="workspace">
      <header><div className="crumb">RECALL WORKSPACE <span>/</span> ACTIVE INVESTIGATION</div><div className="header-actions"><button className="outline" onClick={() => loadDashboard({ refresh: true })} disabled={refreshing}>{refreshing ? "Refreshing..." : "Refresh data"}</button><button className="outline" onClick={exportBrief}>Export brief</button></div></header>
